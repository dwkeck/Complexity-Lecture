...
min = -10
max = 10
timeInterval = 0.105 #Possibly timeLimit / timeInterval bounded at 100?
timeLimit = 5
...
#     geom_path() +
#     transition_states(Time, transition_length=1, state_length=1, wrap=F) +
#     geom_point(x=timeData$OptimalX, y=timeData$OptimalY, colour="hotpink", size=4) +
#     labs(title = "Time: {closest_state}") +
#     enter_fade() + 
#     exit_fade()
```
...
The function updates the OptimalX and OptimalY columns of a dataframe containing X, Y, and Time to include the best results from simulated annealing optimization function. It attempts to find the maxima.

ticksPerUnitTime controls how fast the simulated annealing runs compared to the function. For every increase in the time value of 1.0, the simulated annealing will update its value approximately ticksPerUnitTime times (possibly 1 less, due to rounding).

At time=0, will always use a single random guess. That is, do not expect the first value to be a good one.

costFunction is the function being optimized, of the form y = costFunction(x, time)

Additional parameters should be self-explanatory with a basic understanding of simulated annealing.
```{r}
simulatedAnnealing <- function(timeDataFrame, ticksPerUnitTime, costFunction, startX = NULL)
{
  # Setup variables
  ...
  {
    # Calculate number of simulated annealing ticks to do
    ...
    getOption <- function() { 
      return (runif(1, 
                    max(bestX - 1, min(currentSubset$X)), 
                    min(bestX + 1, max(currentSubset$X))))
    }
    bestY = costFunction(bestX, currentTime)
...
timeData = simulatedAnnealing(timeData, 9, f2D, startX = -10)
visualizeFunction(timeData)

# Approximately 3 guesses per tick
timeData = simulatedAnnealing(timeData, 28, f2D, startX = -10)
visualizeFunction(timeData)

# Approximately 3 ticks per guess
timeData = simulatedAnnealing(timeData, 3, f2D, startX = -10)
visualizeFunction(timeData)
```
